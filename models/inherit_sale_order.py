...
class InheritedSaleOrder(models.Model):
    ...
    available_credit = fields.Float(
        string='Crédito Dispoible', compute='_compute_available_credit', readonly=True, copy=False, store=True)
    ...
    def create(self, vals):
        ...
        partner = self.env['res.partner'].search(
            [('id', '=', vals['partner_id'])])
        if partner.credit_enabled and partner.available_credit - self.credit_after_sale > 0:
